...
class FindCorrespondences(object):
    def __init__(self, pickle_file, correspondences_pickle_file):
        ...
        self.pitch_correction = -.039      # -.039 is indicated by Tango API, -.1 seems to give better alignment
        ...
        self.im1 = None
        self.im2 = None
        self.curr_im1_idx = 542 # -1 TODO: change back
        self.curr_im2_idx = 435 # -1
        ...
        if self.im1_plane != None:
            ...
            if self.im1_pts:
                if do_correction:
                    ...
                    #print [g(x) for x in np.linspace(-.5,.5,20)]
    ...
    def correct_roll_and_pitch(self, transformation_matrix, K=None, im=None, points=None, pixel_coordinates=None):
        ...
        roll, pitch, yaw = euler_from_matrix(transformation_matrix)
        pitch += self.pitch_correction
        rotation = euler_matrix(-pitch, 0, roll-pi)

        # TODO: experiment with not correcting here, but instead correcting later
        #rotation = np.eye(4)
    ...
    def handle_click_im(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            # change axes of the 3d points to conform to OpenCV's coordinate axes conventions
            depth_camera_axes = im_depth_cloud[[1,2,0],:]
